...
try {
  ...
  console.warn('[WARN] [cache] 缓存目录创建失败:', e.message);
}
...
function readLocalCache(symbol) {
  ...
  try {
    ...
    console.error(`[ERROR] [cache] 读取失败 ${symbol}:`, err.message);
    return null;
  }
}
...
function saveLocalCache(symbol, rawData) {
  try {
    ...
    console.error(`[ERROR] [cache] 写入失败 ${symbol}:`, err.message);
  }
}
...
function saveBackupCache(symbol, data) {
  try {
    ...
    console.error(`[ERROR] [backup] 写入失败 ${symbol}:`, e.message);
  }
}
...
function fetchYahooRaw(symbol, callback, opts = {}) {
      ...
      try {
        ...
        if (parsed?.chart?.result) {
          ...
          if (!opts.skipTencentFallback && symbol === '000688.SS' && (!res0.timestamp || res0.timestamp.length <= 5)) {
            console.warn('[WARN] [yahoo] 000688.SS 历史点数不足，使用腾讯财经 1200 天备选源补全...');
        ...
        console.warn(`[WARN] [yahoo] ${symbol} 无数据${desc ? ': ' + desc : ''} (HTTP ${r.statusCode})`);
      } catch (e) {
        console.error(`[ERROR] [yahoo] ${symbol} 响应解析失败:`, e.message);
      }
      callback(null);
    });
  });
  ...
  req.on('error', (err) => {
    console.error(`[ERROR] [yahoo] ${symbol} 网络错误:`, err.message);
    callback(null);
  });
  req.setTimeout(YAHOO_TIMEOUT, () => {
    req.destroy();
    console.warn(`[WARN] [yahoo] ${symbol} 超时 (${YAHOO_TIMEOUT}ms)`);
    callback(null);
  });
}
...
function mergeYahooIncremental(cachedContent, incContent) {
  try {
    ...
    if (updated || appended) {
      console.log(`[INFO] [yahoo] 增量合并: 更新 ${updated} 点, 追加 ${appended} 点`);
    }
    return JSON.stringify({ chart: { result: [base], error: null } });
  } catch (e) {
    console.warn('[WARN] [yahoo] 增量合并失败:', e.message);
    return null;
  }
}
...
function refreshYahoo(symbol) {
  ...
  fetchYahooRaw(symbol, (incRaw) => {
    ...
    if (merged) {
      ...
      console.warn(`[WARN] [yahoo] ${symbol} 增量合并失败，回退全量抓取`);
  ...
  const req = https.get(sinaUrl, {
    ...
    r.on('end', () => {
      try {
        ...
        console.log(`[INFO] [sina] ${symbol} → ${code}: ${count} 个数据点`);
        const merged = saveBackupMerged(symbol, map);
        callback(merged);
      } catch (e) {
        console.warn(`[WARN] [sina] ${symbol} 解析失败:`, e.message);
        callback(null);
      }
    });
  });

  req.on('error', e => { console.warn(`[WARN] [sina] ${symbol} 网络错误:`, e.message); callback(null); });
  req.setTimeout(BACKUP_TIMEOUT, () => { req.destroy(); console.warn(`[WARN] [sina] ${symbol} 超时`); callback(null); });
}
...
function parseYahooToDateMap(yahooContent) {
  try {
    ...
    const cls = q.close || [];
    // 日内高低：恐慌指数「日内波幅」彩带需要（上边界 high、下边界 low）。
    // 磁盘缓存本来就存了完整 OHLC，此前只是在这一步被丢弃。
    const his = q.high || [];
    const los = q.low || [];
    const num = (a, i) => (a[i] != null && !isNaN(a[i])) ? a[i] : null;
    const map = {};
    tss.forEach((t, i) => {
      const date = new Date(t * 1000).toISOString().split('T')[0];
      map[date] = { t, v: num(cls, i), h: num(his, i), l: num(los, i) };
    });
    return map;
  } catch (e) {
    return null;
...
function fetchEastmoneyData(symbol, limit, callback) {
  ...
  if (until && Date.now() < until) {
    console.log(`[INFO] [eastmoney] ${symbol} 冷却中，跳过 (剩余 ${Math.round((until - Date.now()) / 1000)}s)`);
    callback(null);
    return;
  }
  ...
  const fail = (reason) => {
    ...
    console.warn(`[WARN] [eastmoney] ${symbol} ${reason}，进入 ${BACKUP_COOLDOWN_MS / 60000} 分钟冷却`);
    callback(null);
  };
  ...
  const req = http.get(emUrl, {
    ...
    r.on('end', () => {
      try {
        ...
        if (klines.length === 0) {
          console.warn(`[WARN] [eastmoney] ${symbol} → ${secid}: 无数据（secid 可能失效）`);
          callback(null);
          return;
        }
        ...
        const count = Object.keys(map).length;
        if (count === 0) { callback(null); return; }
        console.log(`[INFO] [eastmoney] ${symbol} → ${secid}: ${count} 个数据点`);
        const merged = saveBackupMerged(symbol, map);
        callback(merged);
      } catch (e) {
        fail(`解析失败 (${e.message})`);
  ...
  Array.from(allDates).sort().forEach(date => {
    ...
    if (yV != null && bV != null) {
      ...
      if (diffPct > 1.0) {
        ...
        console.warn(`[WARN] ⚠️  [校验] ${symbol} ${date}: Yahoo=${yV.toFixed(2)} 备选=${bV.toFixed(2)} Δ${diffPct.toFixed(2)}%`);
      }
    }
    ...
    // 日内高低只有主源（Yahoo）有；备选源仅日收盘，退回 close 使彩带退化成零宽，
    // 视觉上等同于没有当日波幅，不会画出错误的宽带。
    const h = yEntry?.h ?? bEntry?.h ?? finalV;
    const l = yEntry?.l ?? bEntry?.l ?? finalV;

    const t = yEntry?.t || bEntry?.t;
    if (t) series.push({ t, v: finalV, h, l, source, date });
  });
...
function buildMerged(symbol, callback) {
  ...
  function send(yahooMap, backupMap, fromCache) {
    ...
    if (missingCt || backupCt) {
      console.log(`[INFO] [merged] ${symbol} 数据说明（非报错）: 缺失 ${missingCt} 点（多为休市日）${backupCt ? `, 备选源扩展历史 ${backupCt} 点` : ''}, 校验偏差 ${crossValidation.discrepancies.length} 处`);
    }
    callback(
  ...
  if (yahooCache) {
    console.log(`[INFO] [merged] ⚡ 磁盘缓存命中: ${symbol} (age ${Math.round(yahooCache.ageMs / 1000)}s)`);
  ...
  const breaker = setTimeout(() => {
    console.warn(`[WARN] [merged] ⏰ 冷启动 ${COLD_START_MS}ms 熔断: ${symbol}`);
...
/**
 * 轻量实时接口：只返回最近 n 个交易日的点（实时轮询用，避免全量传输）。
 * - 读磁盘缓存尾部（秒回），若缓存过期则后台触发 5d 增量刷新，
 *   下次轮询即可拿到新值；本次仍返回已有数据（保证响应不阻塞）。
 * - 增量刷新由 refreshYahoo 处理（range=5d 合并进 10y 全量缓存），
 *   前端下一次轮询会拿到更新后的尾部。
 */
function buildLatest(symbol, n = 3, callback) {
  const cache = readLocalCache(symbol);
  if (!cache) {
    // 无缓存：冷启动，触发全量抓取后回调空（前端保持现状等下一轮）
    fetchYahooRaw(symbol, () => { });
    callback({ symbol, latest: [], fromCache: false });
    return;
  }
  if (!cache.isValid) refreshYahoo(symbol);   // 过期 → 后台增量刷新

  const yahooMap = parseYahooToDateMap(cache.content);
  const backupCache = readBackupCache(symbol);
  const { series } = mergeAndValidate(yahooMap, backupCache?.data || null, symbol);
  const latest = series.filter(p => p.v != null).slice(-n);
  callback({ symbol, latest, fromCache: true });
}
