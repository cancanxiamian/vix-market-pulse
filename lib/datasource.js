...
function parseYahooToDateMap(yahooContent) {
  try {
    ...
    tss.forEach((t, i) => {
      ...
      map[date] = {
        t,
        v: (cls[i] != null && !isNaN(cls[i])) ? cls[i] : null,
        // 日内最高：恐慌指数取色用（getVixColor(high)），位置仍用 close。
        // 股指无需 high，但透传无害；备份源无 high 时保持 undefined。
        h: (highs[i] != null && !isNaN(highs[i])) ? highs[i] : null,
      };
    });
    return map;
  } catch (e) {
...
function mergeAndValidate(yahooMap, backupMap, symbol) {
  ...
  Array.from(allDates).sort().forEach(date => {
    ...
    let finalV = null, finalH = null, source = 'missing';
    if (yV != null) { finalV = yV; source = 'yahoo'; }
    else if (bV != null) { finalV = bV; source = 'backup'; }

    // 日内最高：主源优先（Yahoo 有 high），备份源没有 high 时退回 close 值（视觉无差异）
    const yH = yEntry?.h;
    const bH = bEntry?.h;
    if (yH != null) finalH = yH;
    else if (bH != null) finalH = bH;
    else if (finalV != null) finalH = finalV;

    const t = yEntry?.t || bEntry?.t;
    if (t) series.push({ t, v: finalV, h: finalH, source, date });
  });
