...
function fetchYahooRaw(symbol, callback) {
  const yahooUrl = `https://query1.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(symbol)}`
    + `?range=${YAHOO_RANGE}&interval=1d&events=history&includePrePost=false`;
    ...
    r.on('end', () => {
      try {
        ...
        if (parsed?.chart?.result) {
          ...
          if (symbol === '000688.SS' && (!res0.timestamp || res0.timestamp.length <= 5)) {
...
// 成功时回写备份缓存并回调日期映射，失败回调 null
function fetchSinaData(symbol, callback) {
  ...
  const sinaUrl = 'https://money.finance.sina.com.cn/quotes_service/api/json_v2.php'
    + `/CN_MarketData.getKLineData?symbol=${code}&scale=240&ma=no&datalen=3000`;
...
function fetchBackupData(symbol, callback) {
  if (SINA_MAP[symbol]) {
    fetchSinaData(symbol, (map) => {
      if (map) { callback(map); return; }
      if (EASTMONEY_MAP[symbol]) fetchEastmoneyData(symbol, callback);
      else callback(null);
    });
    return;
  }
  fetchEastmoneyData(symbol, callback);
}

function fetchEastmoneyData(symbol, callback) {
  ...
  const emUrl = `http://push2his.eastmoney.com/api/qt/stock/kline/get?secid=${secid}`
    + `&fields1=f1,f2,f3,f4,f5,f6&fields2=f51,f52,f53,f54,f55&klt=101&fqt=1&end=20500101&lmt=3000`;
...
function buildMerged(symbol, callback) {
  ...
  if (yahooCache) {
    ...
    if (!yahooCache.isValid) fetchYahooRaw(symbol, () => {});
 ...
 *
 * @param {boolean} force 为 true 时忽略 Yahoo 缓存有效期强制拉取；
 *                        备选源始终尊重自己的 6 小时 TTL，不受 force 影响。
 */
function refreshAll(force = false) {
  for (const symbol of ALL_SYMBOLS) {
    if (force || !readLocalCache(symbol)?.isValid) fetchYahooRaw(symbol, () => {});
  }
